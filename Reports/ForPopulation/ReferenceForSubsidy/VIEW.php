...
class VIEW extends \Reports\reportView
{
    ...
    {
        $D = $this->dataLS;
        $this->insertValue(1,1,"Лицевой Сч.№ ".$D['id_LS']);
        $this->insertValue(2,1,$this->DH['name_organization']);
        $this->insertValue(3,1,"ИНН/КПП ".$this->DH['INN']."/".$this->DH['KKP']);
        $this->insertValue(4,1,"№ лицензии  от ".$this->DH['OGRN']);
        $this->insertValue(5,1,"ОГРН ".$this->DH['OGRN']);
        $this->insertValue(6,1,"Тел. приемная ".$this->DH['name_organization']);
        $this->insertValue(7,1,"Абонентский сектор ".$this->DH['name_organization']);
        $this->insertValue(8,1,"Адрес:  {$D['status_street']} {$D['name_street']} д.{$D['house']} кв.{$D['room']}");

        $this->insertValue(23,1,"Лицевой Сч.№ ".$D['id_LS']);
        $this->insertValue(24,1,$this->DH['name_organization']);
        $this->insertValue(25,1,"Адрес:  {$D['status_street']} {$D['name_street']} д.{$D['house']} кв.{$D['room']}");
        ...
        $this->insertTable(3,11,11,$this->dataLS['table'],array(3=>"name_type_accrual",6=>"value",7=>"tarif1",8=>"summa",9=>"recalc",10=>"coefficient",11=>"total"));
