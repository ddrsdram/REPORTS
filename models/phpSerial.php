...
class phpSerial
{
    ...
    function confBaudRate ($rate)
    {
        ...
        if (isset($validBauds[$rate]))
        {
            if ($this->_os === "linux")
            {
                $cmd = "stty -F " . $this->_device . " " . (int) $rate;
                print $cmd;
                $ret = $this->_exec($cmd, $out);
            }
            if ($this->_os === "osx")
            {
    ...
    function readPort ($count = 0)
    {
        ...
        if ($this->_os === "linux" || $this->_os === "osx")
        {
            ...
            {
                do {
                    print "Read from COM-Port".chr(10).chr(13);
...
//                    $content .= fread($this->_dHandle, 128);
    ...
    {
        trigger_error("exec command = ".$cmd, E_USER_WARNING);
        ...
        $ret = stream_get_contents($pipes[0]);
...
/*
        trigger_error("retVal : " . $retVal, E_USER_WARNING);
        trigger_error("ret : " . $ret, E_USER_WARNING);
        trigger_error("err : " . $err, E_USER_WARNING);
*/
